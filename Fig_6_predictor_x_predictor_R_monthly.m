% Script to generate Figure 6 in Heidemann et al. 2024
...
% option for different time periods 

% calculate timeseries (reshaped into months) for all possible predictors
% to see how they are correlated

% time_period = '1920-2023'
% time_period = '1940-2023'
% time_period = '1957-2023'
time_period = '1975-2023'

% 1)) get SST indices for predictors  --> works for all time periods
 [Nino34_std,CP_std,EP_std,DMI_std,Coral_std,Timor_std,...
     Arafura_std,IOBW_std,TPI_filt,Ningaloo_std,C_std,E_std]=get_SST_indices_func(time_period); % '1920-2023'

 % 2) get feedbacks and other non-SST based indices --> depending on the time periods output
 % varies
...
% 3) get MJO (only when analysing 1975 to 2023), no data available for
% earlier periods
...
% get rid of one half of the matrix, as its just duplicated the same values
...
elseif num_predictors ==17 % 1957
yticklabels({'','CP Index','EP Index','Nino 3.4 lag-1','Timor Sea','Arafura Seas','Coral Sea',...
    'DMI','IOBW','TPI lowpass filtered','Ningaloo Nino','Soil moisture lag-1 NE',...
    'Soil moisture lag-1 NW','ET lag-1 NE','ET lag-1 NW',...
    'NW oceanic evaporation','TBO','AM SAM'})
elseif num_predictors ==16 % 1940
yticklabels({'','CP Index','EP Index','Nino 3.4 lag-1','Timor Sea','Arafura Seas','Coral Sea',...
    'DMI','IOBW','TPI lowpass filtered','Ningaloo Nino','Soil moisture lag-1 NE',...
    'Soil moisture lag-1 NW','ET lag-1 NE','ET lag-1 NW',...
    'NW oceanic evaporation','TBO'})
else
    num_predictors==14; % 1920
yticklabels({'','CP Index','EP Index','Nino 3.4 lag-1','Timor Sea','Arafura Seas','Coral Sea',...
    'DMI','IOBW','TPI lowpass filtered','Ningaloo Nino','Soil moisture lag-1 NE',...
    'Soil moisture lag-1 NW','ET lag-1 NE','ET lag-1 NW'})
...
for col = 1:num_predictors
    ...
    if P_matrix(row,col,m_idx)<=0.05 % set p value for display of significance here 
    ...
    cb=colorbar
    cb.Position = [0.88 0.29 0.017 0.45]; %adjust!
    cb.Label.String = 'r';  
    cb.FontSize=12;
...
% works best by manually making window larger to fit whole screen first
% when working with second screen 
