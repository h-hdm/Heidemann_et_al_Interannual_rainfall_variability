% Figure 7 in Heidemann et al. (2024)
% 
...
% set(gcf,'color','w');
% 
% fname_str = ['Stacked_bar_plots_upd']
% print(fname_str,'-dtiff','-r300')

%% next load data for display of seasonal cycle - use monthly means! also for 1975 to 2023
...
if strcmp('Northeast',region)
    lon_idx = idx_east;
    rainfall_NE_clim = nanmean(Rainfall_climatology(lon_idx,lat_idx,:,:),1);
    rainfall_NE_clim=squeeze(nanmean(rainfall_NE_clim,2));
    % area averaged rainfall 
    rainfall_NE = nanmean(rainfall_land(lon_idx,lat_idx,:,:),1);
    rainfall_NE=squeeze(nanmean(rainfall_NE,2));

    % standard deviation and standard error per month 
    rainfall_NE_std = zeros(size(rainfall_NE_clim));
    rainfall_NE_sem = zeros(size(rainfall_NE_clim));
    for i_dx = 1:12
    rainfall_NE_std(i_dx,:)=std(rainfall_NE(i_dx,:)); % standard devation
    rainfall_NE_sem(i_dx,:)= rainfall_NE_std(i_dx,:)./sqrt(size(rainfall_NE,2)); % standard error 
    ...
    lon_idx = idx_west;
    rainfall_NW_clim = nanmean(Rainfall_climatology(lon_idx,lat_idx,:,:),1);
    rainfall_NW_clim=squeeze(nanmean(rainfall_NW_clim,2));
    ...
    % standard deviation and standard error per month
    rainfall_NW_std = zeros(size(rainfall_NW_clim));
    rainfall_NW_sem = zeros(size(rainfall_NW_clim));
    for i_dx = 1:12
    rainfall_NW_std(i_dx,:)=std(rainfall_NW(i_dx,:)); % standard devation
    rainfall_NW_sem(i_dx,:)= rainfall_NW_std(i_dx,:)./sqrt(size(rainfall_NW,2)); % standard error 
    end
...
% recenter the data 
% NW
rainfall_NW_clim1 = rainfall_NW_clim(10:12);
rainfall_NW_clim2 = rainfall_NW_clim(1:4);
rainfall_NW_clim_Oct_Apr = [rainfall_NW_clim1;rainfall_NW_clim2];

rainfall_NW_std1 = rainfall_NW_std(10:12);
rainfall_NW_std2 = rainfall_NW_std(1:4);
rainfall_NW_std_Oct_Apr = [rainfall_NW_std1;rainfall_NW_std2];

rainfall_NW_sem1 = rainfall_NW_sem(10:12);
rainfall_NW_sem2 = rainfall_NW_sem(1:4);
rainfall_NW_sem_Oct_Apr = [rainfall_NW_sem1;rainfall_NW_sem2];


% NE
rainfall_NE_clim1 = rainfall_NE_clim(10:12);
rainfall_NE_clim2 = rainfall_NE_clim(1:4);
rainfall_NE_clim_Oct_Apr = [rainfall_NE_clim1;rainfall_NE_clim2];

rainfall_NE_std1 = rainfall_NE_std(10:12);
rainfall_NE_std2 = rainfall_NE_std(1:4);
rainfall_NE_std_Oct_Apr = [rainfall_NE_std1;rainfall_NE_std2];

rainfall_NE_sem1 = rainfall_NE_sem(10:12);
rainfall_NE_sem2 = rainfall_NE_sem(1:4);
rainfall_NE_sem_Oct_Apr = [rainfall_NE_sem1;rainfall_NE_sem2];
...
ha = fill([3,3,6,6],[0 400 400 0],rgb('light blue')) % DJFM
ha.FaceAlpha =  0.4;
ha.EdgeColor = "none"

hold on 
plot(rainfall_NW_clim_Oct_Apr,'k','LineWidth',1.5)


hold on 
e=errorbar((rainfall_NW_clim_Oct_Apr),rainfall_NW_std_Oct_Apr,'.','MarkerSize',10,...
    'MarkerEdgeColor','red','MarkerFaceColor','blue','LineWidth',1);
...
hold on 
plot(rainfall_NE_clim_Oct_Apr,'k','LineWidth',1.5)
hold on 

e=errorbar((rainfall_NE_clim_Oct_Apr),rainfall_NE_std_Oct_Apr,'.','MarkerSize',10,...
    'MarkerEdgeColor','red','MarkerFaceColor','blue','LineWidth',1);
...
legend('monsoon season','\mu','\pm\sigma','Location','northwest','FontSize',8)
...
% set position and size of graphs 
set(a_sub,'Position',[0.165 0.59 0.3 0.27]);
set(b_sub,'Position',[0.625 0.59 0.3 0.27]);
...
fname_str = ['Figure_7_Stacked_bar_plots_upd']
print(fname_str,'-dtiff','-r300')
